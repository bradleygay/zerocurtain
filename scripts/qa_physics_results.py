...
def quality_assurance_checks(parquet_file):
    ...
    for feature in main_features:
        ...
        if feature == 'intensity_percentile':
            ...
            if out_of_range > 0:
                print(f"    {feature}: {out_of_range} values outside [0,1]")
        ...
        if feature == 'duration_hours':
            ...
            if negative > 0:
                print(f"    {feature}: {negative} negative values")
    ...
    if 'latitude' in df.columns:
        ...
        if invalid_lat > 0:
            print(f"    {invalid_lat} invalid latitude values")
    ...
    if 'longitude' in df.columns:
        ...
        if invalid_lon > 0:
            print(f"    {invalid_lon} invalid longitude values")
    ...
    if 'start_time' in df.columns and 'end_time' in df.columns:
        ...
        if invalid_times > 0:
            print(f"    {invalid_times} events with end_time < start_time")
...
def main():
    outputs_dir = Path("/path/to/user/arctic_zero_curtain_pipeline/outputs")
