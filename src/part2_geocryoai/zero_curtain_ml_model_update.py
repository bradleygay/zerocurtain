...
class LazyDiskDataset(Dataset):
    """Load sequences from disk on-demand."""
    def __init__(self, batch_files, sequence_length, sequence_counts=None):
        self.batch_files = batch_files
        self.sequence_length = sequence_length
        self.index = []
        
        if sequence_counts is not None:
            # Use cached counts - NO FILE LOADING
            for batch_idx, count in enumerate(sequence_counts):
                for i in range(count):
                    self.index.append((batch_idx, i))
        else:
            # Fallback: read ONLY array shapes, not data
            for batch_idx, batch_file in enumerate(batch_files):
                with np.load(batch_file, allow_pickle=True) as data:
                    # Get count from array shape without loading data
    ...
    def __getitem__(self, idx):
        batch_idx, seq_idx = self.index[idx]
        data = np.load(self.batch_files[batch_idx], allow_pickle=True)
        return (torch.FloatTensor(data['features'][seq_idx]), 
                torch.FloatTensor(data['targets'][seq_idx]), 
                int(data['lengths'][seq_idx]))
...
class ZeroCurtainDataset(Dataset):
    ...
    def __init__(self,
        ...
        # CACHED SEQUENCE CREATION
        import os, pickle, hashlib
        cache_dir = "/discover/nobackup/bagay/part2_discover_package/outputs/part2_geocryoai/sequence_cache"
        os.makedirs(cache_dir, exist_ok=True)
        data_hash = hashlib.md5(f"{self.df.shape}_{sequence_length}".encode()).hexdigest()[:8]
        batch_dir = os.path.join(cache_dir, f"batches_{data_hash}_{len(self.df)}")
        cache_file = os.path.join(cache_dir, f"meta_{data_hash}_{len(self.df)}.pkl")
        
        if os.path.exists(cache_file):
            print(f" LOADING CACHED SEQUENCES")
            with open(cache_file, 'rb') as f:
                cache_data = pickle.load(f)
            self.batch_files = cache_data['batch_files']
            self.sequence_counts = cache_data.get('sequence_counts', None)
            self.use_lazy_loading = True
            self.lazy_dataset = LazyDiskDataset(self.batch_files, sequence_length, self.sequence_counts)
            print(f"  {len(self.lazy_dataset):,} cached sequences loaded")
            self.sequences = []
            self.sequence_targets = []
            self.sequence_lengths = []
        else:
            print(" CREATING CACHED SEQUENCES")
            os.makedirs(batch_dir, exist_ok=True)
            
            # CHECK FOR EXISTING BATCHES (RESUME CAPABILITY)
            import glob
            existing_batches = sorted(glob.glob(os.path.join(batch_dir, 'batch_*.npz')))
            batch_files = []
            sequence_counts = []
            
            if existing_batches:
                print(f"  Found {len(existing_batches)} existing batches - RESUMING")
                batch_files = existing_batches
                # Load counts from existing batches
                for bf in batch_files:
                    with np.load(bf, allow_pickle=True) as data:
                        sequence_counts.append(data['features'].shape[0])
                batch_num = len(existing_batches)
                print(f"  Loaded {sum(sequence_counts):,} existing sequences")
                print(f"  Starting from batch {batch_num}")
            else:
                print("  No existing batches - creating from scratch")
                batch_num = 0
            
            for batch_start in range(batch_num * 25, len(list(self.df.groupby('site_id'))), 25):
                batch_sites = list(self.df.groupby('site_id'))[batch_start:batch_start+25]
                batch_data = {'features': [], 'targets': [], 'lengths': []}
                
                for site_id, group in batch_sites:
                    group_indices = group.index.tolist()
                    if len(group_indices) < sequence_length:
                        continue
                    for i in range(len(group_indices) - sequence_length + 1):
                        seq_idx = group_indices[i:i + sequence_length]
                        batch_data['features'].append(self.features[seq_idx])
                        batch_data['targets'].append(self.targets[seq_idx[-1]])
                        batch_data['lengths'].append(sequence_length)
                
                if batch_data['features']:
                    batch_file = os.path.join(batch_dir, f'batch_{batch_num}.npz')
                    np.savez_compressed(batch_file, **batch_data)
                    batch_files.append(batch_file)
                    sequence_counts.append(len(batch_data['features']))
                    batch_num += 1
                    print(f"    Batch {batch_num} saved")
                    
                    # CRITICAL: Free batch_data immediately
                    del batch_data
                    import gc
                    gc.collect()
            
            # Save with counts (already tracked during creation)
            with open(cache_file, 'wb') as f:
                pickle.dump({'batch_files': batch_files, 'sequence_counts': sequence_counts}, f)
            
            self.batch_files = batch_files
            self.use_lazy_loading = True
            # Use counts tracked during creation (no reloading)
            self.lazy_dataset = LazyDiskDataset(batch_files, sequence_length, sequence_counts)
            print(f"  {len(self.lazy_dataset):,} sequences created and cached")
            self.sequences = []
            self.sequence_targets = []
            self.sequence_lengths = []
    ...
    def __len__(self):
        """Return the number of sequences in the dataset"""
        if hasattr(self, 'use_lazy_loading') and self.use_lazy_loading:
    ...
    def __getitem__(self, idx):
        ...
        if self.enable_pattern_analysis and idx < len(self.temporal_patterns) and self.temporal_patterns[idx] is not None:
            item['temporal_pattern'] = torch.LongTensor(self.temporal_patterns[idx])
        
        return item['features'], item['targets'], item['sequence_length']
...
def load_and_preprocess_data(parquet_file):
    ...
    if not Path(parquet_file).exists():
        ...
        alternatives = [
            ...
            Path(__file__).parent.parent.parent / 'outputs/part1_pinszc' / Path(parquet_file).name,
        ...
        if not found:
            ...
            print(f"  - outputs/part1_pinszc/zero_curtain_enhanced_cryogrid_physics_dataset.parquet")
...
def create_data_loaders(df, batch_size=128, sequence_length=90, 
                       train_ratio=0.7, val_ratio=0.15, 
                       enable_pattern_analysis=False,
                       num_workers=None):
    # Calculate test_size and val_size from train_ratio and val_ratio
    test_size = 1.0 - train_ratio - val_ratio
    val_size = val_ratio  # val_size is the same as val_ratio
    
    """
    Create train/val/test data loaders with site-based splitting.
    
    Args:
        num_workers: Number of dataloader workers (auto-detect if None)
    """
    
    # Auto-detect optimal number of workers
    if num_workers is None:
        import os
        num_workers = min(os.cpu_count() or 1, 8)  # Cap at 8 for stability
    ...
    # CRITICAL: Delete train and val datasets BEFORE creating test
    # Test is the only one that needs to create batches
    ...
    # CRITICAL MEMORY CLEANUP: Delete datasets after DataLoaders created
    print("Freeing val/test dataset memory...")
    del val_dataset
    del test_dataset
    import gc
    gc.collect()
    print(" Val/test datasets deleted - only loaders and train_dataset remain")
...
def calculate_optimal_sequence_length(df, target_temporal_coverage='seasonal'):
    ...
    if 'start_time' in df.columns:
        ...
        # Safety check for zero or invalid median_gap
        if median_gap == 0 or np.isnan(median_gap) or median_gap < 0.1:
            print(f"  WARNING: Median time gap is {median_gap} hours (invalid)")
            median_gap = 24.0
            print(f"  Setting default median gap to 24.0 hours")
...
def main(config_path='part2_config_update.yaml'):
    """
    ...
    # Load configuration
    from config_loader_update import load_config, get_default_config
